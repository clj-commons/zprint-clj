...
program
  .version(pkg.version)
  .option('-c, --check "<pattern>"', 'Checks formatting without writing to output, zprint-clj -c "./out/**/*.{clj,cljs,cljc,edn}"')
  .option('-i, --input "<pattern>"', 'Input file, directory or glob pattern. If no output specified writes to stdout.')
...
if (program.input || program.check) {
  ...
  const paths = fg.sync(program.input || program.check, { onlyFiles: true, dot: false });
  ...
  const outs = srcs.map(([p, src]) => format(src, p, opts));
  ...
  if (program.check) {
    ...
    if (diffs.length > 0) {
      ...
      console.log('Code style issues found in the above file(s). Forgot to run ZPrint?');
      process.exit(1);
    } else {
      console.log('All matched files use ZPrint code style!');
    }
  } else if (program.out) {
    Promise.all(outs.map((out, idx) => new Promise((resolve, reject) => {
      const pout = path.join(program.out, srcs[idx][0]);
      fs.writeFile(pout, out, 'utf8', err => err ? reject(err) : resolve());
    })))
    .then(() => {
      console.log('Done.')
    })
